...
AS $function$
  ...
  CASE WHEN percentage THEN
      ((price - (select price from serum_events where timestamp < now() - duration::interval and serum_events.type = 'trade' ORDER BY serum_events.timestamp desc LIMIT 1))/(select price from serum_events where timestamp < now() - duration::interval and serum_events.type = 'trade' ORDER BY serum_events.timestamp desc LIMIT 1)) * 100
  ELSE
